...
$$ 
The spatial field $W(s)$ captures autocorrelation not explained by the SCORPAN factors. The latent process $\eta(s)$ will then be used in the observation-likelihood, which is here chosen as gaussian. We use the `|` notation to indicate conditioning of the property at the left side of `|` on the parameters given to the right side of `|`. This leads to the following hierarchical formulation for the observations, 
