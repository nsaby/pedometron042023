...
We construct a hierarchical modelfor the soil property $z(s)$ of a spatial location $s$ in the framework of the prediction in the presence of exhaustive ancillary information. In this paper, $z$ will correspond to the organic matter, `om`. We will assume the following linkage between model components and observations, where we denote the latent process by $\eta(s)$.
