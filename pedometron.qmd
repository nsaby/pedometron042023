...
One need to indicate the likelihood family such as `gaussian`, `poisson` or `binomial`. By default family is `gaussian`. A list of possible alternatives can be seen by typing `names(inla.models()$likelihood)`. It is therefore possible to fit a wide range of model allowing to approach a great diversity of problems in soil science. It is for example possible to fit use the `gamma` family in this example to cope with heavy tail.

Heuvelink and Webster [-@HEUVELINK2022100639]  listed a set of challenges for pedometricians and spatial statisticians  to strengthen the role of spatial statistics. Without being able to solve all of them, it seems to us that `INLA` by providing a fully Bayesian modelling in a rapid and continent way can provide some answers to some of them, e.g., the better uncertainty quantification, the change of support, incorporating attribute and positional measurement uncertainty. 
