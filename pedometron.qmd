...
$$
The spatial field $W(s)$ captures autocorrelation not explained by the covariates. The latent process $\eta(s)$ will then be used in the observation likelihood, which is here chosen as gamma distribution as soil carbon is known to present a relatively heavy-tailed distribution.
...
We can plot the spatial surfaces corresponding to the pointwise median and lower and upper bounds of a 95% credible interval as follows (assuming that the predicted intensity is in the object `pred`).
...
Heuvelink and Webster [-@HEUVELINK2022100639]  listed a set of challenges for pedometricians and spatial statisticians  to strengthen the role of spatial statistics and fully exploit cutting-edge tools provided by its state-of-the-art. Without being able to solve all of them, we are convinced that fully Bayesian modelling using `INLA`  with  its ergonomic and numerically highly efficient implementation can provide answers to some of these challenges, in particular improved uncertainty quantification,  change of the support, and the incorporation of attribute and positional measurement uncertainty. 
